...
test("newly registered run exposes launch selection before its journal event exists", () => {
  ...
  const actorModelOverrides = [{ name: "writer", selection }];
  const entry = {
  ...
  const journal = {
    getRun: () => undefined,
    getEvents: () => {
      throw new Error("journal must not be read before launch");
    },
  ...
  assert.deepEqual(snapshot?.actorModelOverrides, actorModelOverrides);
});
