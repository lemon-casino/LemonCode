...
**Snippet tool call contract:** `EvalWorkflowSnippet` accepts exactly one source. Use
`{ code: "..." }` for an inline snippet **or** `{ path: "relative/or/absolute/file.ts" }`
for a snippet file. Never pass both fields and never omit both. The tool is optional: use it
only when testing fixed parser, glob, gate, or command-output logic is valuable. It is not
a required step before `CreateWorkflow`. When a caller explicitly asks for `/lemon`, skip
`EvalWorkflowSnippet` and submit the workflow directly with one `CreateWorkflow` call whose
only workflow source is `script`.
