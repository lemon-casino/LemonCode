...
- The task supplement reuses the ordinary `ChatPromptEditor` shell and composer attachment transaction. It accepts text plus image-only picker/paste input. Pending images render as the same removable thumbnail treatment as the main composer inside that shell, before the editable text; they are never rendered as a separate row outside the input. Images are uploaded/staged before submission, represented by bounded `AttachmentRef` values in the task command and journal, and delivered as image blocks to the selected actor's next attempt. A completed ask's revision carries the references to its new run; cold resume replays the same references. An upload failure or missing image keeps the draft and does not send a text-only substitute. Unrelated tasks and the main composer do not receive the image.
...
- The actor tab has a compact task selector and status/actions header (Stop, Rerun, effective model/reasoning/speed), scrollable read-only transcript with preserved attempts, and a bottom supplementary composer using the same input shell, editor spacing, attachment thumbnails and submit treatment as the main conversation. Actor-only capability differences remain explicit: the action menu accepts images only and does not expose model, mode, mention or slash controls. Stop is available only while an ask is live; Rerun is available for live, stopped, failed and completed asks. All controls have accessible labels and tooltips.
...
10. Paste an image and optional text into a selected actor task. The image appears as a removable main-composer-style thumbnail inside the shared input shell above the text, submit waits for upload, and the selected actor receives an image on retry/revision. The picker rejects non-image files. `Ctrl+Enter` / `Cmd+Enter` and the send button share the same admission path. Switching task scope restores that task's text/image draft without sending it elsewhere; on command rejection, upload error or attachment preparation failure both text and image remain available and the submit control becomes usable again. Only an accepted ACK clears the submitted draft, and a late rejection/error remains attached to the task that issued the command instead of appearing on the newly focused task. Cold resume of an accepted revision retains bounded image refs without duplicating the command.
...
- UI interaction tests cover per-ask focus, action enablement, shared composer-shell structure, in-shell image thumbnails, failure/permission handling, narrow widths and keyboard access. The actor-side E2E scenario selects an ask, pastes an image, verifies the thumbnail is contained by the input shell, types text, submits through the modified-Enter path, and verifies rejected commands preserve both inputs while an accepted ACK clears only that ask's draft. Run package tests, `pnpm typecheck`, `pnpm lint`, and `pnpm architecture:check --changed` after implementation.
